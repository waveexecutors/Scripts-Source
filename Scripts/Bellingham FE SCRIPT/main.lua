...
-- Delta Executor Mobile Optimized (Fixed Slide + Scrolling GUI + Custom Intro)
...
mainFrame.BackgroundColor3 = Color3.fromRGB(43, 43, 43) -- Darcula-style dark base
...
-- Added a subtle neon glow border
...
-- Converted to ScrollingFrame so all elements fit properly
...
contentFrame.CanvasSize = UDim2.new(0, 0, 0, 300) -- Expands scrolling area
...
-- Automatically update CanvasSize based on items added
uiList.GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    contentFrame.CanvasSize = UDim2.new(0, 0, 0, uiList.AbsoluteContentSize.Y + 15)
end)

-- Spacer to push the first button down slightly
...
-- ==========================================
-- SAFE DRIFT SYSTEM (Fixed slide, removed spin/fling)
-- ==========================================
...
local function performDrift()
    ...
    -- Smooth physics state without the aggressive fling
    hum:ChangeState(Enum.HumanoidStateType.Physics)
    
    local slideVelocity = Instance.new("BodyVelocity")
    slideVelocity.MaxForce = Vector3.new(25000, 0, 25000) -- Smoother force
    slideVelocity.Velocity = root.CFrame.LookVector * 55 -- Fixed slide speed
...
createButton("⚡ Smooth Slide", performDrift, Color3.fromRGB(200, 150, 50))
...
-- Added Textbox for Intro Speed
local delayInput = Instance.new("TextBox")
delayInput.Size = UDim2.new(0, 150, 0, 30)
delayInput.BackgroundColor3 = Color3.fromRGB(35, 35, 45)
delayInput.TextColor3 = Color3.fromRGB(255, 255, 255)
delayInput.PlaceholderText = "Intro Speed (Default: 0.7s)"
delayInput.Text = ""
...
diCorner.CornerRadius = UDim.new(0, 8)
...
delayInput.FocusLost:Connect(function()
    ...
    if num then
        introDelay = num
        delayInput.Text = tostring(num) .. "s"
    else
        delayInput.Text = ""
        introDelay = 0.7
    end
...
-- Replay Intro Button
createButton("🔄 Replay Intro", playIntro, Color3.fromRGB(150, 50, 200))
...
print("⚽ Bellingham FE Loaded! (Fixed UI + Smooth Drift)")
