...
-- Delta Executor Mobile Optimized (Fixed Drift Physics & Video Player)
...
-- Drift Function (Controlled slide to prevent flying out of the map)
local function performDrift()
    ...
    -- Controlled Slide Forward (Keeps you glued to the ground)
    local slideVelocity = Instance.new("BodyVelocity")
    slideVelocity.MaxForce = Vector3.new(100000, 100000, 100000)
    -- Push forward, but apply a slight downward pull (-15 on Y axis) to prevent flying
    local lookVector = root.CFrame.LookVector
    slideVelocity.Velocity = Vector3.new(lookVector.X * 75, -15, lookVector.Z * 75)
    slideVelocity.Parent = root
    
    -- Fling spinner (Spins you on the Y axis safely to trip others without glitching out)
    local spinFling = Instance.new("BodyAngularVelocity")
    spinFling.MaxTorque = Vector3.new(100000, 100000, 100000)
    spinFling.AngularVelocity = Vector3.new(0, 75, 0) 
    ...
    connection = game:GetService("RunService").Stepped:Connect(function(_, deltaTime)
        ...
        -- After 1.5 seconds, stop sliding
        if driftTime >= 1.5 then
            connection:Disconnect()
        end
    end)
    
    -- Wait for the drift to finish
    task.wait(1.5)
    
    -- Cleanup Physics safely
    if slideVelocity then slideVelocity:Destroy() end
    if spinFling then spinFling:Destroy() end
    
    -- Reset velocities to stop instantly
...
-- INTRO SYSTEM - Fixed IsPlaying error & Added Timeout
    ...
    if videoData then
        ...
        if writefile and customAssetFunc then
            ...
            if success then
                ...
                -- Timeout implementation: Prevent infinite hang if executor fails to decode .webm
                local loadAttempts = 0
                local maxAttempts = 30 -- Wait up to 3 seconds for video to be ready
                
                while not video.IsLoaded and loadAttempts < maxAttempts do
                    task.wait(0.1)
                    loadAttempts = loadAttempts + 1
                end
                
                -- Play the video ONLY if it successfully loaded
                if video.IsLoaded then
                    video:Play()
                    
                    -- Wait for video to finish (FIX: Changed IsPlaying to Playing)
                    local waitTime = 0
                    while video.Playing and waitTime < 8 do
                        task.wait(0.1)
                    ...
                    warn("Video took too long to load, skipping intro.")
                end
