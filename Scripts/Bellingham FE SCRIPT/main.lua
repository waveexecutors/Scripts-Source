...
-- Intro System - Delta Executor specific
...
function downloadVideo()
    ...
    -- Download using Delta's HTTP service
    ...
    if success then
        -- Save to workspace
        videoFile = Instance.new("StringValue")
        videoFile.Name = "IntroVideo"
        videoFile.Value = response
        videoFile.Parent = workspace
        return true
    end
    return false
end
...
function playIntro()
    if not videoFile then
        -- Download first
        local downloaded = downloadVideo()
        if not downloaded then
            print("Failed to download intro video")
            return
        end
    end
    ...
    -- Play using Delta's VideoPlayer
    local videoPlayer = Instance.new("VideoPlayer")
    videoPlayer.Size = UDim2.new(1, 0, 1, 0)
    videoPlayer.Parent = videoFrame
    
    -- Load from workspace
    local videoData = videoFile.Value
    
    -- Play the video
    local success = pcall(function()
        -- For Delta, we need to use the raw data
        videoPlayer:PlayFromData(videoData)
        loadingText:Destroy()
    end)
    
    if not success then
        -- Alternative method for Delta
        pcall(function()
            videoPlayer:Play("https://github.com/waveexecutors/Scripts-Source/raw/refs/heads/main/Scripts/Bellingham%20FE%20SCRIPT/lv_0_20260723141904.webm")
            loadingText:Destroy()
        end)
    end
