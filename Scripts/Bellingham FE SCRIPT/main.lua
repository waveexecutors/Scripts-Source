...
local blockDelay = 0.7 -- Default delay between blocks
...
local function buildIntroBlocks(container, delay)
...
local function playIntro()
    task.spawn(function()
        ...
        -- Build the blocks with current delay
        local blocks = buildIntroBlocks(container, blockDelay)
        ...
        -- Animation Sequence: Slide down every X seconds (user controlled)
        local slideTweenInfo = TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local currentDelay = blockDelay -- Use the current value
...
print("💡 Use the textbox to change intro block speed (0.7 default)")
