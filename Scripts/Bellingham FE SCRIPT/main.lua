...
-- Replay Intro Button - Now properly references the function
local replayBtn = createButton("🔄 Replay Intro", function()
    playIntro()
...
local blockDelay = 0.7 -- Default delay between blocks (for blocks 2, 3, 4)
...
-- Separate function to build intro blocks (reusable)
local function buildIntroBlocks(container)
    ...
    local function createBlock(yPos)
        local frame = Instance.new("Frame")
        frame.Name = "IntroBlock"
        frame.Size = UDim2.new(1, 0, 0.25, 0)
        frame.Position = UDim2.new(0, 0, -0.25, 0) -- Starts off-screen
...
-- Store intro GUI reference for replay
local currentIntroGui = nil

local function playIntro()
    -- If there's already an intro playing, destroy it first
...
print("💡 Other blocks use the speed you set (default 0.7s)")
