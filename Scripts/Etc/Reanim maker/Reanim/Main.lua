--[[ R6 Reanim Creator - Compact Small | Motor6D only ]]
...
main.Size,main.Position,main.BackgroundColor3,main.BorderSizePixel,main.ClipsDescendants = UDim2.new(0,270,0,390),UDim2.new(.5,-135,.5,-195),Color3.fromRGB(22,22,30),0,true
...
content.Size,content.Position,content.BackgroundTransparency,content.BorderSizePixel,content.ScrollBarThickness,content.ScrollBarImageColor3,content.CanvasSize =
	UDim2.new(1,0,1,-36),UDim2.new(0,0,0,36),1,0,3,Color3.fromRGB(100,90,180),UDim2.new(0,0,0,560)
...
local function xyzRow(lab,y)
	local r = Instance.new("Frame",xSec) r.Size,r.Position,r.BackgroundTransparency = UDim2.new(1,-16,0,26),UDim2.new(0,8,0,y),1
	local l = Instance.new("TextLabel",r) l.Size,l.BackgroundTransparency,l.Text,l.Font,l.TextSize,l.TextColor3,l.TextXAlignment =
		UDim2.new(0,36,1,0),1,lab,Enum.Font.Gotham,11,Color3.fromRGB(200,195,230),Enum.TextXAlignment.Left
	local boxes = {}
	for i,ax in ipairs({"X","Y","Z"}) do
		...
		tb.Size,tb.Position,tb.BackgroundColor3,tb.Text,tb.Font,tb.TextSize,tb.TextColor3,tb.PlaceholderText,tb.ClearTextOnFocus =
			UDim2.new(0,58,0,22),UDim2.new(0,40+(i-1)*62,.5,-11),Color3.fromRGB(40,38,60),"0",Enum.Font.Gotham,12,Color3.new(1,1,1),ax,false
		Instance.new("UICorner",tb).CornerRadius = UDim.new(0,5)
		boxes[ax] = tb
	end
	return boxes
end
local posB,rotB = xyzRow("Pos",22),xyzRow("Rot",50)
...
sLab.Size,sLab.Position,sLab.BackgroundTransparency,sLab.Text,sLab.Font,sLab.TextSize,sLab.TextColor3 =
	UDim2.new(0,42,0,24),UDim2.new(0,8,0,22),1,"Speed",Enum.Font.Gotham,11,Color3.fromRGB(200,195,230)
...
speedBox.Size,speedBox.Position,speedBox.BackgroundColor3,speedBox.Text,speedBox.Font,speedBox.TextSize,speedBox.TextColor3 =
	UDim2.new(0,50,0,24),UDim2.new(0,50,0,22),Color3.fromRGB(40,38,60),"1",Enum.Font.Gotham,12,Color3.new(1,1,1)
...
kfLab.Size,kfLab.Position,kfLab.BackgroundTransparency,kfLab.Text,kfLab.Font,kfLab.TextSize,kfLab.TextColor3 =
	UDim2.new(0,100,0,24),UDim2.new(0,110,0,22),1,"KF: 1/1",Enum.Font.Gotham,11,Color3.fromRGB(200,195,230)
...
local function stopPrev()
	previewing=false prevBtn.Text,prevBtn.BackgroundColor3 = "▶ Preview (OFF)",Color3.fromRGB(40,120,90)
...
local function startPrev()
	...
	killAnimate(c) storeOrig(c)
	previewing=true prevBtn.Text,prevBtn.BackgroundColor3 = "⏹ Preview (ON)",Color3.fromRGB(180,70,60)
	local t=0
...
local function genScript()
	...
	local function ser(kf)
		local t={} for p,d in pairs(kf) do table.insert(t,string.format('["%s"]={pos=Vector3.new(%.2f,%.2f,%.2f),rot=Vector3.new(%.2f,%.2f,%.2f)}',p,d.pos.X,d.pos.Y,d.pos.Z,d.rot.X,d.rot.Y,d.rot.Z)) end
	...
	outBox.Text = [[-- R6 Reanim (Motor6D) | No Animate
...
local function setup(c)
	...
	RS.Heartbeat:Connect(function(dt)
		...
		t=t+dt*data.speed local idx,nidx,al=math.floor(t%#kfs)+1,(math.floor(t%#kfs)%#kfs)+1,t%1
...
-- Auto kill Animate on character
...
saveBtn.MouseButton1Click:Connect(function() saveBoxes() anims[curAnim].speed=tonumber(speedBox.Text)or 1 saveBtn.Text="✓ Saved" task.delay(1,function() saveBtn.Text="💾 Save" end) end)
...
-- Min / Close
local mini=false
minBtn.MouseButton1Click:Connect(function() mini=not mini content.Visible=not mini main.Size=mini and UDim2.new(0,270,0,36) or UDim2.new(0,270,0,390) minBtn.Text=mini and "+" or "–" end)
